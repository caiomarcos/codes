# -*- coding: utf-8 -*-
"""
Created on Sat Aug  8 21:57:34 2020
...
# %%
ser = serial.Serial('COM3', baudrate=230400, timeout=None)
ser.flushInput()
ser.close()
ser.open()
# %%
...
for slic in Sl_test:
    ...
    print("slice sent, waiting result")
    for x in range(6):
        ieee754_data = ser.read(4)
        my_float, = struct.unpack('f', ieee754_data)
        result.append(my_float)
...
results_df = pd.DataFrame.from_records(results2)
...
predictions2 = (results_array > 0.5)
...
history = model2.fit(X_train, y_train, batch_size=10, nb_epoch=100)
                     # validation_data=(X_test, y_test))
# %%
# # # Sl_test = np.float32(Sl_test)
# # print(struct.pack('f', Sl_test[2,3]))
# # ieee754_data = struct.pack('f', Sl_test[2,3])
# # print(ieee754_data)
# # print(Sl_test[2,3])
# # # ser.write(ieee754_data)
# # # %%

# # print(X_test[1684, 27, 27, 1])
...
#     pickle.dump([confusion_matrices_1, accuracies_1, precisions_1, recalls_1,
#                  f1s_1], f)
